...
class YseUserdataManager implements YseUserdataManagerInterface {
   ...
   *   The user data service.
     */
    public function __construct(ConfigFactoryInterface $config_factory, YseUserdataPluginManager $plugin_manager, LoggerInterface $logger_interface,  CacheBackendInterface $cache_service, UserDataInterface $user_data) {
    ...
    protected function calculateHash($lookupkey) {
      ...
      if (!$this->streamWrapperManager->isValidlookupkey($lookupkey) && !$this->lookupkeysystem->realpath($lookupkey)) {
        return NULL;
      }
