 ...
 *
 * @Action(
 *  id = "yse_discourse_deleteuser_action",
 *  label = @Translation("YSE Users: SSO Sync with YSE Discourse"),
 *  type = "user",
 *  confirm = TRUE
 * )
 */
final class YseDiscourseDeleteuserAction extends ViewsBulkOperationsActionBase {
  ...
  public function execute($account = NULL) {
    ...
    if (!empty($discourse_extid) && is_numeric($discourse_extid)){
      ...
      $discourse_confirmation = \Drupal::service('yse_accountmanager.discourseutils')->delete_user($parameters, $account->id());
      ...
      if (!empty($discourse_confirmation)){
        if ($discourse_confirmation == 'DELETED'){
            return $this->messenger()->addMessage('Discourse user deleted for ' . $account->getDisplayName() . ' as ' . $discourse_confirmation . '. Success.');
        } 
        else {
            return $this->messenger()->addMessage('Discourse user anonymised for ' . $account->getDisplayName() . ' as ' . $discourse_confirmation . ' to retain their posts. Success.');
        }
      }
    } else {
      return  $this->messenger()->addWarning('Failed to delete with Discourse for ' . $account->getDisplayName() . '. Skipping.');
    }
